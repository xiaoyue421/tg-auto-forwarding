...
async def send_text_to_targets_detailed(
    client: TelegramClient,
    text: str,
    targets: list[tuple[ForwardTarget, object]],
    logger: logging.Logger,
...
async def send_text_to_bot_targets_detailed(
    ...
    tasks = [
        asyncio.create_task(
            _send_text_to_single_bot_target_chain(
                bot_contexts=bot_contexts,
                text=text,
                target=target,
                logger=logger,
...
async def _send_text_to_single_target(
    client: TelegramClient,
    text: str,
    ...
    try:
        await _send_plain_text_with_retry(
            client=client,
            entity=entity,
            text=text,
            target=target,
        ...
        logger.exception("text send failed -> %s", target.chat)
        ...
        logger.exception("unexpected text send failure -> %s", target.chat)
        return TargetDispatchResult(
            channel="account",
            target_chat=target.chat,
            success=False,
...
async def _send_text_to_single_bot_target_chain(
    bot_contexts: list[BotForwardContext],
    text: str,
    target: ForwardTarget,
    logger: logging.Logger,
    ...
    for index, context in enumerate(bot_contexts):
        ...
        attempted = True
        try:
            await _send_bot_plain_text_with_retry(
            ...
            if index > 0:
                logger.info(
                    "bot text send succeeded via %s -> %s",
                    context.label,
                    target.chat,
                    extra={"monitor": True},
                )
            ...
            monitor_log(
                logger,
                logging.WARNING,
                "Bot 文本发送失败，尝试下一个 Bot",
                message=message,
                target=target.chat,
                context=log_context,
            )
            logger.warning(
                "bot text send failed via %s -> %s: %s",
                context.label,
                target.chat,
                exc,
            )
            ...
            monitor_log(
                logger,
                logging.WARNING,
                "Bot 文本发送异常，尝试下一个 Bot",
                message=message,
                target=target.chat,
                context=log_context,
            )
            logger.warning(
                "bot text send unexpected via %s -> %s: %s",
                context.label,
                target.chat,
                exc,
            )
        ...
        if index < last_index:
            logger.warning(
                "bot text send failed via %s for -> %s, trying next bot",
                context.label,
                target.chat,
                extra={"monitor": True},
            )
    ...
    if attempted:
        return TargetDispatchResult(
            ...
            target_chat=target.chat,
            success=False,
            error_message=last_error or "bot text send failed",
    ...
    logger.warning(
        "no available bot context for text send target=%s",
        target.chat,
    )
    return TargetDispatchResult(
        channel="bot",
...
async def _send_plain_text_with_retry(
    *,
    client: TelegramClient,
    entity: object,
    text: str,
    ...
    random_id = _build_stable_random_id("account-text", idempotency_key) if idempotency_key else None
    try:
        ...
        logger.warning(
            "FloodWait for %s seconds before text retry, target=%s",
            exc.seconds,
            target.chat,
            extra={"monitor": True},
        )
...
async def _send_bot_plain_text_with_retry(
    *,
    ...
    random_id = _build_stable_random_id("bot-text", idempotency_key) if idempotency_key else None
    try:
        if random_id is None:
        ...
        logger.warning(
            "Bot FloodWait for %s seconds before text retry, target=%s",
            exc.seconds,
            target.chat,
            extra={"monitor": True},
        )
