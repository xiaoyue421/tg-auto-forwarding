...
async def manual_forward_message_async(
    config_path: str | Path,
    source_chat: str,
    message_id: int,
    target_chats: str,
...
def _bot_pool_start_stagger_seconds() -> float:
    raw = (os.getenv("TG_BOT_POOL_START_STAGGER_SECONDS") or "3.5").strip()
    try:
...
async def build_bot_forward_contexts(
    ...
    *,
    ...
    stagger = _bot_pool_start_stagger_seconds()
    for index, bot_token in enumerate(settings.bot_tokens, start=1):
        if index > 1 and stagger > 0:
            if logger is not None:
                logger.info(
                    "%s staggering bot login: sleep %.1fs before %s",
                    log_scope,
                    stagger,
                    f"bot#{index}",
                    extra={"monitor": True},
